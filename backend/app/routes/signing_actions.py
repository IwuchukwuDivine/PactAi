...
@router.post("/contracts/{contract_id}/signatures/{signature_id}/decline", summary="Decline a signature link", responses={400: {"model": ErrorResponse}, 500: {"model": ErrorResponse}})
def decline_signature(contract_id: str, signature_id: str, payload: dict = Body(...)):
    ...
    if not reason:
        raise HTTPException(status_code=400, detail={"error": "Bad Request", "detail": "Missing rejection reason"})
    try:
