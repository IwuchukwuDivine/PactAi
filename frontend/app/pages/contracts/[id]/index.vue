...
Generated by PactAI${ref ? " — " + ref : ""}
</div></body></html>`;

  const printWindow = window.open("", "_blank");
  if (!printWindow) {
    addToast("info", "Please allow popups to download the PDF.");
    return;
  }
...
const handleShare = async () => {
  const url = `${window.location.origin}/sign/${route.params.id}`;
  try {
    ...
    addToast("error", "Failed to copy link. Please copy it manually: " + url);
  }
};
